...
namespace core::renderer
{
    ...
    void Renderer::renderer_init(FrameContext* p_frame_context)
    {
        ...
        //TODO: std::unique_ptr is used in the header class. Remove it to get rid of the error
        auto pass = init_pass(pass_id, true);
        pass.create_command_pool()
        .create_depth_stencil_image(pass_id);

        for (int i = 0; i < max_frames_in_flight; i++)
        {
            pass.allocate_command_buffer(pass_id, i);
        }

        for (int i = 0; i < max_frames_in_flight; i++)
        {
            pass.cache_active_command_buffer(pass_id, i)
            .begin_command_buffer_recording()
            .set_image_layout_transitions_for_active_command_buffer(pass_id, i)
            .setup_attachments(pass_id, i)
            //TODO: Pass function through a param
            .record_draw_batches([&](){ draw_frame(); })
    ...
    VkCommandBuffer* Renderer::get_command_buffer(uint32_t pass_id, uint32_t image_id)
    {
    ...
    Renderer& Renderer::cache_active_command_buffer(uint32_t pass_id, uint32_t image_id)
    {
        active_command_buffer = get_command_buffer(pass_id, image_id);
        return *this;
    }

    Renderer& Renderer::init_pass(uint32_t pass_id, bool use_max_frames)
    {
        if (use_max_frames)
        {
            for (int i = 0; i < max_frames_in_flight; i++)
    ...
    Renderer& Renderer::create_command_pool()
    {
        utils::RenderUtils::create_command_pool(*render_context, command_pool);
        return *this;
    }

    Renderer& Renderer::create_depth_stencil_image(uint32_t pass_id)
    {
    ...
    Renderer& Renderer::allocate_command_buffer(uint32_t pass_id, uint32_t image)
    {
    ...
    Renderer& Renderer::begin_command_buffer_recording()
    {
    ...
    Renderer& Renderer::set_image_layout_transitions_for_active_command_buffer(uint32_t pass_id, uint32_t image)
    {
#ifdef _DEBUG
        assert(active_command_buffer);
#endif
    ...
    Renderer& Renderer::setup_attachments(uint32_t pass_id, uint32_t image)
    {
#ifdef _DEBUG
        assert(active_command_buffer);
#endif
    ...
    Renderer& Renderer::record_draw_batches(const std::function<void()>& func)
    {
        func();
        return *this;
    }

    Renderer& Renderer::end_command_buffer_recording(uint32_t image)
    {
        ...
        if (render_context->dispatch_table.endCommandBuffer(command_buffers[i]) != VK_SUCCESS)
        {
